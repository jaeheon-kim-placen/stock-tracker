  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="bg-gray-900 border-b border-gray-800 px-6 py-4 flex justify-between items-center">
        <h1 className="text-xl font-bold">?ìà Stock Tracker</h1>
        <div className="flex gap-3">
          <button onClick={() => router.push('/admin')} className="text-gray-400 hover:text-white text-sm">Í¥ÄÎ¶¨Ïûê</button>
          <button onClick={handleLogout} className="text-gray-400 hover:text-white text-sm">Î°úÍ∑∏?ÑÏõÉ</button>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto p-6">
        <div className="flex gap-2 mb-6">
          ...
          <button onClick={() => setTab('portfolio')} className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors ${tab === 'portfolio' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>
            ?íº ???¨Ìä∏?¥Î¶¨??{holdings.length > 0 && <span className="ml-1 bg-blue-500 text-white text-xs px-1.5 rounded-full">{holdings.length}</span>}
          </button>
        </div>
          ...
          <>
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
                <p className="text-gray-400 text-sm">?ÑÏ≤¥ ?§Îçî</p>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              {hiddenOrderIds.size > 0 && (
                <button onClick={() => setShowHidden(!showHidden)} className="text-gray-400 hover:text-white text-xs px-3 py-1.5 bg-gray-800 rounded-lg">
                  {showHidden ? 'Î™©Î°ù ?ëÍ∏∞' : `ÏßÄ????™© Î≥¥Í∏∞ (${hiddenOrderIds.size})`}
                </button>
              )}
            </div>
            ...
            {loading ? (
              <div className="text-center text-gray-500 py-20">Î∂àÎü¨?§Îäî Ï§?..</div>
            ) : (
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          {profit ? (
                            ...
                            <p className="text-gray-600 text-xs">Í∞ÄÍ≤?Ï°∞Ìöå Ï§?..</p>
                          )}
                          {inPortfolio ? (
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-800 grid grid-cols-2 gap-2 text-xs">
                        {profit?.orderPrice && (
                          <p className="text-gray-400">
                            ?§Îçî ?πÏãú Í∞ÄÍ≤? <span className="text-yellow-400 font-medium">{formatWithKRW(profit.orderPrice)}</span>
                            {profit.isHistorical && <span className="text-gray-600 ml-1">(?πÏùº Ï¢ÖÍ?)</span>}
                          </p>
                        )}
                        {order.quantity && (
                          <p className="text-gray-400">?òÎüâ: <span className="text-white font-medium">{order.quantity}</span></p>
                        )}
          ...
          <div>
            {totalStats && (
              <div className="bg-gray-900 rounded-xl p-5 border border-gray-700 mb-6">
                <p className="text-gray-400 text-sm mb-3">?ìä ?ÑÏ≤¥ ?¨Ìä∏?¥Î¶¨???©ÏÇ∞</p>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-gray-500 text-xs mb-1">?¨Îü¨ Í∏∞Ï? ?òÏùµÎ•?/p>
                  ...
                  <div>
                    <p className="text-gray-500 text-xs mb-1">?êÌôî Í∏∞Ï? ?òÏùµÎ•?(?òÏ∞®???¨Ìï®)</p>
                ...
                <div className="mt-3 pt-3 border-t border-gray-800 grid grid-cols-2 gap-2 text-xs text-gray-500">
                  <p>?¨Ïûê?êÍ∏à: {formatUSD(totalStats.totalInvestUSD)} / {formatKRW(totalStats.totalInvestKRW)}</p>
                  <p>?ÑÏû¨Í∞ÄÏπ? {formatUSD(totalStats.totalCurrentUSD)} / {formatKRW(totalStats.totalCurrentKRW)}</p>
                  <p className="col-span-2">?ÑÏû¨ ?òÏú®: 1 USD = {formatKRW(exchangeRate)}</p>
                </div>
              </div>
            )}
            ...
            {holdings.length === 0 ? (
              <div className="text-center text-gray-500 py-20">
                <p className="text-4xl mb-3">?íº</p>
                <p>Î≥¥Ïú† Ï¢ÖÎ™©???ÜÏñ¥??/p>
                <p className="text-sm mt-2">?§Îçî ?ºÎìú?êÏÑú "+ ??Í±∞Îûò Ï∂îÍ?" Î≤ÑÌäº???åÎü¨Î≥¥ÏÑ∏??/p>
              </div>
            ) : (
              <div className="space-y-3">
                    ...
                    <div key={holding.stock_code} className="bg-gray-900 rounded-xl p-5 border border-gray-800">
                      ...
                      <div className="mt-4 grid grid-cols-3 gap-3 text-sm">
                        <div className="bg-gray-800 rounded-lg p-3">
                          <p className="text-gray-400 text-xs mb-1">Î≥¥Ïú† ?òÎüâ</p>
                          <p className="font-medium">{holding.total_quantity.toLocaleString(undefined, {maximumFractionDigits: 4})} Ï£?/p>
                        </div>
                        <div className="bg-gray-800 rounded-lg p-3">
                          <p className="text-gray-400 text-xs mb-1">?âÍ∑† ?®Í?</p>
                          <p className="font-medium text-sm">{holding.avg_price ? formatWithKRW(holding.avg_price) : '-'}</p>
                        </div>
                        <div className="bg-gray-800 rounded-lg p-3">
                          <p className="text-gray-400 text-xs mb-1">?¨Ïûê?êÍ∏à</p>
                      ...
                      {profit && (
                        <div className="mt-3 grid grid-cols-2 gap-2">
                          <div className="bg-gray-800 rounded-lg p-3 text-sm">
                            <p className="text-gray-400 text-xs mb-1">?¨Îü¨ ?âÍ??êÏùµ</p>
                            <p className={`font-bold ${isUSDProfit ? 'text-green-400' : 'text-red-400'}`}>
                              {isUSDProfit ? '+' : ''}{formatUSD(profit.profitUSD)}
                            </p>
                          </div>
                          <div className="bg-gray-800 rounded-lg p-3 text-sm">
                            <p className="text-gray-400 text-xs mb-1">?êÌôî ?âÍ??êÏùµ (?òÏ∞®?µÌè¨??</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 rounded-2xl p-6 w-full max-w-md border border-gray-700">
            <h3 className="font-bold text-lg mb-1">
              {modal.mode === 'buy' ? (modal.order ? '??Í±∞Îûò Ï∂îÍ?' : 'Ï∂îÍ? Îß§Ïàò') : 'Îß§ÎèÑ Ï≤òÎ¶¨'}
            </h3>
            ...
            {modal.historicalPrice && (
              <p className="text-xs text-gray-500 mb-3 bg-gray-800 rounded-lg px-3 py-2">
                ?πÏùº Ï¢ÖÍ?: <span className="text-yellow-400">{formatWithKRW(modal.historicalPrice)}</span>
              </p>
            )}

            <p className="text-xs text-gray-500 mb-1">?ÑÏû¨ ?òÏú®: <span className="text-gray-300">1 USD = {formatKRW(exchangeRate)}</span></p>
            <p className="text-xs text-gray-500 mb-3">????™©Îß??ÖÎ†•?òÎ©¥ ?òÎ®∏ÏßÄÍ∞Ä ?êÎèô Í≥ÑÏÇ∞?ºÏöî</p>

            <div className="space-y-3">
              <div>
                <label className="text-gray-400 text-sm mb-1 block">{modal.mode === 'buy' ? 'Îß§ÏàòÍ∞Ä' : 'Îß§ÎèÑÍ∞Ä'} (1Ï£ºÎãπ USD)</label>
                <input
                  type="number"
                  value={myPrice}
                  onChange={e => { setLastChanged('price'); setMyPrice(e.target.value) }}
                  className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm"
                  placeholder={modal.historicalPrice ? modal.historicalPrice.toString() : "ÏßÅÏ†ë ?ÖÎ†•"}
                />
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-gray-400 text-sm">Í∏àÏï°</label>
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={() => setModal(null)} className="flex-1 bg-gray-700 hover:bg-gray-600 text-white rounded-lg py-2.5 text-sm">Ï∑®ÏÜå</button>
              <button
              ...
              >
                {saving ? '?Ä??Ï§?..' : modal.mode === 'buy' ? (modal.order ? 'Ï∂îÍ??òÍ∏∞' : 'Îß§Ïàò Ï∂îÍ?') : 'Îß§ÎèÑ Ï≤òÎ¶¨'}
              </button>
            </div>
          </div>
        </div>
